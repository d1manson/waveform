...
T.WV = function(CanvasUpdateCallback,TILE_CANVAS_NUM){
    ...
    var Render = function(hRender){
    	...
    	for(var c=0;c<4;c++)if(chanIsOn[c]){ //for each channel TODO: if we have rendered some channels already we should be able to reuse them during this render 

    		//choose the right vertex buffer ..TODO: may be best to iterate through all pages of one channel and then come back and do subsequent channels?
    		...
    		for(var i=firstGroup;i<=lastGroup;i++)if(i< cutIndLengths.length && $tile_[i]){
				...
				if(!$newCanvases[i]){
					$newCanvases[i] = $("<canvas width='" + w + "' height='" + h + "'></canvas>");
					$newCanvases[i].ctx = $newCanvases[i].get(0).getContext('2d');
				}
				$newCanvases[i].ctx.drawImage(Canvas,col*colWidth,row*rowHeight,colWidth-horz,rowHeight,xOff,0,colWidth-horz,rowHeight);
				CanvasUpdateCallback(slotInd,TILE_CANVAS_NUM,$newCanvas); //TODO: slotInd doens't have a value yet..need to pipe it through from the original render event
    		}
    ...
    var CallRender = function(cut,newlyInvalidatedSlots,isNewCut){

		var invalidatedSlots = M.clone(newlyInvalidatedSlots);

		if(cRender && cRender.alive){
			console.log('Killing old render.')
			M.or(invalidatedSlots,cRender.invalidatedSlots,M.IN_PLACE); //invalidatedSlots |= cRender.invalidatedSlots
			cRender.alive = false; 
			cRender = null; //loose the reference to the old cRender object
		}
		cRender = { alive: true,
					invalidatedSlots: invalidatedSlots
					}; 
